...
The `xtractomatic` package is not available through <span style="color:blue">CRAN</span> at the moment, so it must be installed locally from a ".tar.gz" file.  It is recommended that you install the package in your local <span style="color:blue">R</span> directory, rather than the <span style="color:blue">R</span> System directory.  You can find this directory using:

```{r libPaths,eval=FALSE}
.libPaths()
```
 
To install the `xtractomatic` package,

```{r installPack,eval=FALSE}
 install.packages('xtractomatic_3.0.tar.gz',lib='/path-to-your/R/3.1/library',type='source',repos=NULL)
```
...
```{r topotag, fig.align='center',fig.width=6,fig.height = 4}
require("xtractomatic")
require("ggplot2")
ylim<-c(15,30)
xlim<-c(-160,-105)
...
```{r MODISchla,error=TRUE, purl = FALSE}
require(xtractomatic)
require(lubridate)
...
```{r plotChlaTS, fig.width=8,fig.height = 4, fig.align='center',warning=FALSE}
require(ggplot2)
require(ggfortify)
...
```{r plotlogChlaTS, fig.width=8,fig.height = 4, fig.align='center',warning=FALSE}
require(ggplot2)
require(ggfortify)
...
The `searchData` function can be used to search for a subset of that satisfy certain criteria. `searchData` requires a list of lists, where each element of the first item of the list has to be one of `dtypename, datasetname, longname, varname`, and the other is the string to search for in that field. For example to find all of the datasets that have a `varname` that contains `chl` and a `datasetname` that contains `mday` (most monthly datasets have `mday` in their name):
...
which returns among others the three <span style="color:blue">chlorophyll</span> datasets extracted in the `xtracto_3D` section. Note that since the function requires a "list of lists", if only doing the first search the code would be:
...
`searchData` iteratively refines the matches in the order given by the list of list, thus it can be viewed as doing a logical "AND" on the matches.
...
The dataset was specified by the `dtype`, which could be either a number or a character string, and references the information in that dataframe.  The `dtype` names in this version for the most part are same, **BUT THE `dtype` NUMBERS HAVE CHANGED**. If you have used `xtractomatic` before you need to check that the `dtype` information that you are passing is what you think it is.  You can do this using the supplied `getInfo` or `searchData` functions or peruse the `erddapStruct` dataframe.
There are many more datasets available in this version, adn now "last" can be used as time to get the last time period available for that dataset.

### `dtype` as number is deprecated

It is strongly recommended that you move away from passing `dtype` as a number, as the numbers can change and will not be in the next version.  Any <span style="color:blue">ERDDAP</span> dataset has an <span style="color:blue">ERDDAP</span> "dataset ID", as well one or more variables associated with that "dataset ID". Some datasets, such as the <span style="color:blue">ASCAT winds</span> have multiple variables associated with the dataset:

<http://coastwatch.pfeg.noaa.gov/erddap/griddap/erdQAstress1day.html>.

This information is contained in `erddapStruct$datasetname` and `erddapStruct$varname`. Since a single dataset may have more than one variable,  `dtype` as character string is a way to have a unique dataset id and variable name in a single variable.  In the next version, no information about the datasets will be built into the package, and `xtractomatic` will rely on the search and query capabilities of <span style="color:blue">ERDDAP</span>. Then the request will be based on a list of the form  `c(dataset id, varname1,  varname2)`.  This is the approach being used in the <span style="color:blue">ROpenSci</span> projects `rnoaa` and `rerddap`, and we will be making use of parts their code.
