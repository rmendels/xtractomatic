...
alllmons$date <- as.factor(allmons$date)
...
allyears$date <- as.factor(allyears$date)
```
...
The extract (see`str(sanctchl)`) contains two time periods of <span style="color:red">chlorophyll</span> masked for data only in the sanctuary boundaries.  
...
myplot <- myplot + 
    ...
    geom_polygon(data = w, aes(x = long, y = lat, group = group), fill = "grey80") +
    theme_bw() + scale_fill_gradientn(colours = my.col, limits = c(-2, 4.5)) +
